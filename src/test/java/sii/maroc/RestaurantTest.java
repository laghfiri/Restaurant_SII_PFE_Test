...
public class RestaurantTest {
	...
	/**
	 * write a test to ensure that when a recipe require out of stocks ingredients
	 * we receive an UnavailableDishException (unchecked)
	 */
// Allowed modification zone starts here
	@Test
	public void shouldFailWhenOutOfStock() {

		Restaurant restaurant = new Restaurant("1 Kg Flour", "50 tomatoes", "sea salt", "6 balls Mozzarella",
				"olive oil", "water");
		Ticket ticket = restaurant.order("4 Tomato Mozzarella Salad");
		String[] array_mozzarella = restaurant.getMozzarella().split(" ");
		String[] array_tomatos = restaurant.getTomatos().split(" ");
		String[] array_flour = restaurant.getFlour_amount().split(" ");

		if ((Integer.parseInt(array_flour[0]) < 0) || (Integer.parseInt(array_mozzarella[0]) < 0)
				|| (Integer.parseInt(array_tomatos[0]) < 0)) {
			Assert.fail();
		} else {
...
// Allowed modification zone ends here
	...
	/**
	 * when cooking more than one dish of the same type : first dish takes standard
	 * time, subsequents dish have their cooking time halved
	 *
	...
	public void shouldServeMixedOrders() {
		Restaurant restaurant = new Restaurant("1Kg Flour", "50 tomatoes", "sea salt", "6 balls Mozzarella",
				"olive oil", "water");
